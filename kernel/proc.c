...
uint64 dump2(int pid, int register_num, uint64 *return_value) {
    ...
    struct proc *looked_proc;


    for (looked_proc = proc; looked_proc < &proc[NPROC]; looked_proc++) {
        if (looked_proc->pid == pid) {
            goto parent_check;
        }
    }
    return -2;
    ...
    parent_check:
    curr_proc = myproc();
    struct proc *checked_proc = looked_proc;
    while (checked_proc->pid != 1) {
        if (checked_proc->pid == curr_proc->pid) {
            goto debug;
        }
        checked_proc = checked_proc->parent;
    }
    return -1;

    debug:
    if (copyout(curr_proc->pagetable, (uint64) return_value, (char*) (&(looked_proc->trapframe->s2) - 2 + register_num),8) < 0 )  {
        return -4;
    }
